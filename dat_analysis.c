 ...
 * MA 02110-1301, USA.
 * 
 * Read blocks of equal sums from file.
 * Look for Tocta configs in block
 * 
 */
...
int main(int argc, char **argv)
{
	...
	while(!(feof(fin))) {
		...
		fread(equalsums, sizeof(gprime), 4*nRows, fin);
		// Target calc
		Target = equalsums[0][0] + equalsums[0][1] + equalsums[0][2] + equalsums[0][3];
